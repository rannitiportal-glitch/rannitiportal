...
// Firebase से सवाल मंगाने का नया फंक्शन
async function loadQuestionsFromFirebase() {
    try {
        const snapshot = await firebase.firestore().collection("questions").get();
        questionsDB = snapshot.docs.map(doc => {
            let data = doc.data();
            data.id = doc.id; 
            return data;
        });
        console.log(`✅ Firebase Loaded: ${questionsDB.length} questions`);
    } catch (error) {
...
function loginUser() {
    ...
    // ==========================================
    // 👑 1. ADMIN LOGIN (Email Hidden with Secret Code)
    // ==========================================
    // तुम्हारा ईमेल अब एन्क्रिप्टेड (Encrypted) है!
    if (btoa(email.toLowerCase()) === "bW9oYW50aGFrMTY0QGdtYWlsLmNvbQ==") {
        auth.signInWithEmailAndPassword(email, pass)
        .then(() => {
...
function checkPracticeAns(id, sel, ans) {
    ...
    if(sel === ans) { 
        ...
        if(typeof recordAnalytics === 'function') recordAnalytics(true);
    } else { 
        if(sBtn) { 
        ...
        if(typeof recordAnalytics === 'function') recordAnalytics(false);

        // 🔥 AI WEAKNESS ANALYZER FOR PRACTICE
        if(currentUser && currentUser.role !== 'guest') {
            if(!currentUser.weakTopics) currentUser.weakTopics = {};
            // जिस सवाल को गलत किया है, उसका टॉपिक ढूँढना
            let wrongQ = questionsDB.find(q => q.id === id);
            let topicName = wrongQ ? wrongQ.topic : (typeof selectedTopic !== 'undefined' && selectedTopic ? selectedTopic.name : "Mixed");
            
            currentUser.weakTopics[topicName] = (currentUser.weakTopics[topicName] || 0) + 1;
            localStorage.setItem("ranniti_active_user", JSON.stringify(currentUser));
        }
    }
...
function submitMockAnswer(qId, selectedOpt, correctOpt) {
    ...
    let topperTime = Math.floor(Math.random() * 10) + 8; // टॉपर का समय (8 से 18 सेकंड के बीच)
    ...
    // 🔥 GHOST MODE UI: टॉपर का समय दिखाओ
    let msgColor = timeTaken <= topperTime ? "var(--success)" : "var(--warning)";
    let msgText = timeTaken <= topperTime ? `🔥 गज़ब! आपने टॉपर को हरा दिया!` : `⚠️ टॉपर ने इसे ${topperTime}s में किया था।`;
    
    let timeDiv = document.createElement('div');
    timeDiv.innerHTML = `<div style="margin-top:15px; padding:12px; background:var(--bg-color); border:2px dashed ${msgColor}; border-radius:8px; font-weight:bold; color:${msgColor}; text-align:center; font-size:15px; animation: fadeIn 0.5s;">⏱️ ${msgText} (Your Time: ${timeTaken}s)</div>`;
    document.getElementById('mockQuestionArea').firstElementChild.appendChild(timeDiv);

    // अब 1.5 की जगह 3 सेकंड का टाइम देंगे ताकि बच्चा टॉपर का मैसेज पढ़ सके
    setTimeout(() => {
        currentMockIndex++;
        if(typeof renderMockQuestion === 'function') renderMockQuestion();
    }, 3000); 
}
...
window.processBulkImport = function() {
    ...
    qBlocks.forEach(block => {
        ...
        if((qEn || qHi) && a && b && c && d && ans) {
            let qObj = {
                category: cat, topic: topic, subtopic: subcat,
                level: difficulty, medium: medium, 
            ...
            // 🔥 Firebase में सेव
            firebase.firestore().collection("questions").add(qObj)
            .then(() => console.log("सवाल Firebase में सेव हो गया!"))
            .catch((e) => console.log("एरर:", e));
    ...
    if(count > 0) {
        alert(`✅ ${count} Questions Successfully Added to Firebase!`);
        document.getElementById('bulkData').value = "";
        setTimeout(() => { if(typeof loadQuestionsFromFirebase === 'function') loadQuestionsFromFirebase(); }, 2000);
    } else {
...
let questionStartTime = 0; // टाइम ट्रैक करने के लिए
...
window.renderMockQuestion = function() {
    ...
    let q = mockQuestions[currentMockIndex];
    questionStartTime = Date.now(); // 🔥 घड़ी चालू (Timer Start)
...
window.onload = function() {
    loadQuestionsFromFirebase();// सवाल मंगाने वाला आपका पुराना फंक्शन
    checkMagicLinkLogin(); // जैसे ही पेज खुले, चेक करे कि क्या बच्चा ईमेल लिंक से आया है
};
// ==========================================
// ADMIN DELETE CONSOLE LOGIC (FIXED FOR LOCALSTORAGE)
// ==========================================
...
function searchForDelete() {
    ...
    let filteredQs = questionsDB.filter(q => {
        let match = true;
        if(subject && q.category !== subject) match = false; 
        if(topic && q.topic !== topic) match = false;
        if(subtopic && q.subtopic !== subtopic) match = false;
        return match;
    });
    ...
    listDiv.innerHTML = filteredQs.map(q => `
        <div style="border-bottom: 1px solid #eee; padding: 12px 10px; display: flex; justify-content: space-between; align-items: center; gap: 15px;">
            <div>
                <div style="font-size:12px; color:gray; margin-bottom:5px;">
                    <strong>${q.category}</strong> > ${q.topic} > ${q.subtopic}
                </div>
                <div style="font-size:14px; color:#333; line-height: 1.4;">
                    ${(q.q_en || q.q_hi).substring(0, 60)}...
                </div>
            </div>
            <button onclick="confirmAndDelete('${q.id}')" style="background: #ef4444; color: white; border: none; padding: 8px 12px; border-radius: 6px; cursor: pointer; font-size: 13px; font-weight: bold; white-space: nowrap;"><i class='fa-solid fa-trash'></i> Delete</button>
        </div>
    `).join('');
}

async function confirmAndDelete(qId) {
    if (!confirm("⚠️ क्या आप वाकई इस सवाल को हमेशा के लिए डिलीट करना चाहते हैं?")) return;
    ...
    try {
        ...
        alert("❌ डिलीट करने में दिक्कत आई: " + error.message);
    }
}